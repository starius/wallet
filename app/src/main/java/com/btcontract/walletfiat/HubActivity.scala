...
object HubActivity {
  ...
  def requestHostedChannel: Unit = {
    ...
    new HCOpenHandler(LNParams.syncParams.ergveinFiat, randomBytes32, localParams.defaultFinalScriptPubKey, LNParams.cm) {
...
class HubActivity extends NfcReaderActivity with ChanErrorHandlerActivity with ExternalDataChecker with ChoiceReceiver with ChannelListener with CanBeRepliedTo { me =>
  ...
  def INIT(state: Bundle): Unit = {
    if (WalletApp.isAlive && LNParams.isOperational) {
      setContentView(com.btcontract.walletfiat.R.layout.activity_hub)
      instance = me
